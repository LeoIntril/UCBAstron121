...
############################
# 3 Hydrogen Observation with Frequency Switching
############################

# Prepare lists to store block spectra
upper_blocks = []
lower_blocks = []
...
# Loop over nblocks_obs, alternating LO each block for frequency switching
for i in range(nblocks_obs):
    if i % 2 == 0:
        # Upper LO = on-line, Lower LO = off-line
        sdr.center_freq = HI_FREQ - freq_offset
        label = f"HI TARGET Upper LO, block {i+1}"
    else:
        # Lower LO = on-line, Upper LO = off-line
        sdr.center_freq = HI_FREQ + freq_offset
    ...
    # Save each block in the correct list
    if i % 2 == 0:
        upper_blocks.append(power)
    else:
        lower_blocks.append(power)

sdr.close()

# --- Convert lists to arrays and average blocks within each LO ---
spec_upper_blocks = np.array(upper_blocks)
spec_lower_blocks = np.array(lower_blocks)

spec_upper_avg = np.mean(spec_upper_blocks, axis=0)
spec_lower_avg = np.mean(spec_lower_blocks, axis=0)
...
np.savez(os.path.join(outdir, "hi_21cm_final_sdr_longint_freqswitch.npz"),
         ...
         s_cold=spec_cold,
         s_hot=spec_hot,
         spec_upper=spec_upper_avg,
         spec_lower=spec_lower_avg,
